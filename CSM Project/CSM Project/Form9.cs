...
namespace CSM_Project {
    public partial class Form9 : Form {
        ...
        void gridfill() {
            ...
            if (isPurch) {
                carInfoQuery = "SELECT CAR.CAR_NAME,CAR.CAR_ID,CAR.CAR_MODEL,CAR.CAR_COMPANY FROM MANUF_ORDER JOIN STOCK_PAYMENT ON MANUF_ORDER.ORDER_ID = STOCK_PAYMENT.ORDER_ID JOIN CAR ON MANUF_ORDER.CAR_ID = CAR.CAR_ID WHERE STOCK_PAYMENT.ORDER_ID = @id";
                cust_manufQuery = "SELECT MANUFACTURER.MANUFACTURER_NAME, MANUFACTURER.MANUFACTURER_ID, MANUFACTURER.MANUFACTURER_CONTACT, MANUFACTURER.MANUFACTURER_ADDRESS FROM STOCK_PAYMENT JOIN MANUF_ORDER ON STOCK_PAYMENT.ORDER_ID = MANUF_ORDER.ORDER_ID JOIN MANUFACTURER ON MANUF_ORDER.MANUFACTURER_ID = MANUFACTURER.MANUFACTURER_ID WHERE STOCK_PAYMENT.ORDER_ID = @id";
                empInfoQuery = "SELECT EMPLOYEE.EMPLOYEE_ID, EMPLOYEE.EMPLOYEE_NAME, EMPLOYEE.EMPLOYEE_CONTACT, EMPLOYEE.EMPLOYEE_DESIGNATION FROM STOCK_PAYMENT JOIN MANUF_ORDER ON STOCK_PAYMENT.ORDER_ID = MANUF_ORDER.ORDER_ID JOIN EMPLOYEE ON MANUF_ORDER.EMPLOYEE_ID = EMPLOYEE.EMPLOYEE_ID WHERE STOCK_PAYMENT.ORDER_ID = @id";
                orderInfoQuery = "SELECT MANUF_ORDER.ORDER_ID, MANUF_ORDER.BILL, MANUF_ORDER.ORDER_DATE FROM MANUF_ORDER JOIN STOCK_PAYMENT ON STOCK_PAYMENT.ORDER_ID = MANUF_ORDER.ORDER_ID WHERE STOCK_PAYMENT.ORDER_ID = @id";

            }
            else {
                carInfoQuery = "SELECT CAR.CAR_NAME, CAR.CAR_ID, CAR.CAR_MODEL, CAR.CAR_COMPANY FROM CUSTOMER_ORDER JOIN SELL_PAYMENT ON SELL_PAYMENT.ORDER_ID = CUSTOMER_ORDER.ORDER_ID JOIN CAR ON CUSTOMER_ORDER.CAR_ID = CAR.CAR_ID WHERE CUSTOMER_ORDER.ORDER_ID = @id";
                cust_manufQuery = "SELECT CUSTOMER.CUSTOMER_NAME, CUSTOMER.CUSTOMER_CNIC,CUSTOMER.CUSTOMER_CONTACT,CUSTOMER.CUSTOMER_ADDRESS FROM SELL_PAYMENT JOIN CUSTOMER_ORDER ON SELL_PAYMENT.ORDER_ID = CUSTOMER_ORDER.ORDER_ID JOIN CUSTOMER ON CUSTOMER_ORDER.CUSTOMER_CNIC = CUSTOMER.CUSTOMER_CNIC WHERE SELL_PAYMENT.ORDER_ID = @id ";
                empInfoQuery = "SELECT EMPLOYEE.EMPLOYEE_ID, EMPLOYEE.EMPLOYEE_NAME, EMPLOYEE.EMPLOYEE_CONTACT, EMPLOYEE.EMPLOYEE_DESIGNATION FROM SELL_PAYMENT JOIN CUSTOMER_ORDER ON SELL_PAYMENT.ORDER_ID = CUSTOMER_ORDER.ORDER_ID JOIN EMPLOYEE ON CUSTOMER_ORDER.EMPLOYEE_ID = EMPLOYEE.EMPLOYEE_ID WHERE SELL_PAYMENT.ORDER_ID =@id ";
                orderInfoQuery = "SELECT CUSTOMER_ORDER.ORDER_ID, CUSTOMER_ORDER.BILL, CUSTOMER_ORDER.ORDER_DATE FROM CUSTOMER_ORDER JOIN SELL_PAYMENT ON SELL_PAYMENT.ORDER_ID = CUSTOMER_ORDER.ORDER_ID WHERE SELL_PAYMENT.ORDER_ID = @id";
            }
            ...
            carCompanyLbl.Text = Convert.ToString(carInfoData.Tables[0].Rows[0].ItemArray[3]);
            carModelLbl.Text = Convert.ToString(carInfoData.Tables[0].Rows[0].ItemArray[2]);
